...
class Role(db.Model):
    ...
    name = db.Column(db.String(20), unique=True, nullable=False)  # 'student' | 'tutor'

    users = db.relationship("User", back_populates="role_rel")
...
class User(db.Model):
    ...
    role          = db.Column(db.String(20), nullable=False, default="student")  # 'student' | 'tutor'
    created_at    = db.Column(db.DateTime, default=lambda: datetime.now(timezone.utc))

    role_rel     = db.relationship("Role", back_populates="users",
                                   primaryjoin="User.role == foreign(Role.name)",
                                   foreign_keys="User.role")
    submissions  = db.relationship("Submission",    back_populates="student",  cascade="all, delete-orphan")
    reports      = db.relationship("ProgressReport", back_populates="student", cascade="all, delete-orphan")
...
class Course(db.Model):
    ...
    tutor  = db.relationship("User", back_populates="courses")
    topics = db.relationship("SyllabusTopic", back_populates="course", cascade="all, delete-orphan")

    def to_dict(self):
        return {"id": self.id, "title": self.title, "module_code": self.module_code, "tutor_id": self.tutor_id}
...
class Question(db.Model):
    ...
    id             = db.Column(db.String(36), primary_key=True, default=new_uuid)
    topic_id       = db.Column(db.String(36), db.ForeignKey("syllabus_topics.id"), nullable=False)
    question_text  = db.Column(db.Text, nullable=False)
    difficulty     = db.Column(db.String(20), default="medium")  # easy | medium | hard
    ai_model_used  = db.Column(db.String(50))
    generated_at   = db.Column(db.DateTime, default=lambda: datetime.now(timezone.utc))

    topic       = db.relationship("SyllabusTopic", back_populates="questions")
    submissions = db.relationship("Submission", back_populates="question")

    def to_dict(self):
        return {
            "id": self.id, "topic_id": self.topic_id,
            "question_text": self.question_text,
            "difficulty": self.difficulty, "ai_model_used": self.ai_model_used,
        }
...
class Submission(db.Model):
    ...
    feedback_items   = db.relationship("Feedback", back_populates="submission",  cascade="all, delete-orphan")
    ai_model_results = db.relationship("AIModelResult", back_populates="submission", cascade="all, delete-orphan")
...
class Feedback(db.Model):
    ...
    feedback_type = db.Column(db.String(50), default="guided")  # guided | hint | correction
    ai_model_used = db.Column(db.String(50))
    created_at    = db.Column(db.DateTime, default=lambda: datetime.now(timezone.utc))

    submission = db.relationship("Submission", back_populates="feedback_items")

    def to_dict(self):
        return {
            "id": self.id, "submission_id": self.submission_id,
            "feedback_text": self.feedback_text,
            "feedback_type": self.feedback_type, "ai_model_used": self.ai_model_used,
        }
...
class AIModelResult(db.Model):
    ...
    id                = db.Column(db.String(36), primary_key=True, default=new_uuid)
    submission_id     = db.Column(db.String(36), db.ForeignKey("submissions.id"), nullable=False)
    model_name        = db.Column(db.String(50), nullable=False)   # gpt-4o | claude-sonnet | deepseek
    correctness_score = db.Column(db.Float, default=0.0)           # 0–10
    syllabus_score    = db.Column(db.Float, default=0.0)
    quality_score     = db.Column(db.Float, default=0.0)
    consistency_score = db.Column(db.Float, default=0.0)

    submission = db.relationship("Submission", back_populates="ai_model_results")
...
class ProgressReport(db.Model):
    ...
    student = db.relationship("User", back_populates="reports")
